...
#include "goal_factory/open_day_goal_factory.h"
...
// good change to use shared point with auto delete
  ...
  roborts_decision::OpenDayChaseBehavior        open_day_chase_behavior(chassis_executor, trans, full_path);
  roborts_decision::OpenDayPatrolBehavior        open_day_patrol_behavior(chassis_executor, trans, full_path);
  ...
  roborts_decision::OpenDayGoalFactory open_day_goal_factory(chassis_executor, trans, full_path);
  //roborts_decision::AttackBehavior       attack_behavior(chassis_executor, blackboard, full_path);
  //hierarchical_goal_factory.Run();
  open_day_goal_factory.Run();
  ...
  while(ros::ok()){
    ...
    switch (command) {
      ...
      case '1':
        ...
        break;
      case '2':
        open_day_patrol_behavior.Run();
        break;      
      case '3':
        open_day_chase_behavior.Run();
        break;
      case '4':
        goal_behavior.Run();
        break;
...
void Command() {
  ...
  while (command != 27) {
    ...
    std::cout 
              << "1: Open day behavior tree" <<std::endl
              << "2: Patrol behavior" << std::endl
              << "3: Chase behavior" << std:: endl
              << "4: Goal behavoir" << std:: endl
              << "esc: exit program" << std::endl;

    std::cout << "**************************************************************************************" << std::endl;
    std::cout << "> ";
    std::cin >> command;
    if (command != '1' && command != '2' && command != '3' && command != '4' && command != 27) {
