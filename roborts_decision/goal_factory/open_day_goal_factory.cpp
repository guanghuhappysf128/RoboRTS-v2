#include "open_day_goal_factory.h"
#include "test_goal_factory.h"


namespace roborts_decision {
OpenDayGoalFactory::OpenDayGoalFactory(roborts_decision::ChassisExecutor *&chassis_executor,
                                                 const roborts_decision::Blackboard::Ptr &blackboard_ptr,
                                                 const std::string &proto_file_path) :
  blackboard_ptr_(blackboard_ptr),
  root_(new OpenDayRootNode("root", chassis_executor, blackboard_ptr, proto_file_path)),
  behavior_tree_(root_, 100) {
    ctrl_fricWheel_client_ = nh_.serviceClient<roborts_msgs::FricWhl>("cmd_fric_wheel");
  ROS_INFO("OpenDay Goal Factory Done!");
}

void OpenDayGoalFactory::Run() {
  root_->Load();

  ROS_WARN("This code should only been executed once");
  ...
  //Wating for gamestate signal, should be used in the real match
  // ros::Rate r(10);
  // while(ros::ok()) {
  //   ros::spinOnce();
  ...
  // open frictional wheel when this is running 
  // CtrlFricWheel(true);

  // running the behavior_tree
  behavior_tree_.Run();
}
...
OpenDayRootNode::OpenDayRootNode(std::string name, roborts_decision::ChassisExecutor *&chassis_executor,
                                           const roborts_decision::Blackboard::Ptr &blackboard_ptr,
                                           const std::string &proto_file_path) :
...
void OpenDayRootNode::Load() {

  //Instantiate Action nodes
  std::shared_ptr<OpenDayPatrolActionNode> open_day_patrol(new OpenDayPatrolActionNode("open_day_patrol", chassis_executor_, blackboard_ptr_, proto_file_path_));
  std::shared_ptr<OpenDayChaseActionNode> open_day_chase(new OpenDayChaseActionNode("open_day_chase", chassis_executor_, blackboard_ptr_, proto_file_path_));
  ...
  std::shared_ptr<SelectorNode> open_day_root_node(new SelectorNode("open_day_root", blackboard_ptr_));

  std::shared_ptr<PreconditionNode> open_day_pre_chase(new PreconditionNode("open_day_chase", blackboard_ptr_,
      [&]() -> bool { return this->enemy_detected_; }, AbortType::LOW_PRIORITY));
  std::shared_ptr<PreconditionNode> open_day_pre_patrol(new PreconditionNode("open_day_patrol", blackboard_ptr_,
      [&]() -> bool { return !this->enemy_detected_; }, AbortType::LOW_PRIORITY));   
  
  open_day_pre_chase->SetChild(open_day_chase);
  ...
  AddChildren(open_day_root_node);
...
BehaviorState OpenDayRootNode::Update() {
  // Update status flags
...
// Deprecated
bool OpenDayRootNode::HasBullet() {
 ...
 *
 */
OpenDayPatrolActionNode::OpenDayPatrolActionNode(std::string name, roborts_decision::ChassisExecutor *&chassis_executor,
                                   const roborts_decision::Blackboard::Ptr &blackboard_ptr,
                                   const std::string &proto_file_path) :
  ActionNode::ActionNode(name, blackboard_ptr),
  open_day_patrol_behavior_(chassis_executor, blackboard_ptr_, proto_file_path) {}

void OpenDayPatrolActionNode::OnInitialize() {
  //ROS_INFO("%s %s", name_.c_str(), __FUNCTION__);
  //patrol_behavior_.Run();
}

BehaviorState OpenDayPatrolActionNode::Update() {
  open_day_patrol_behavior_.Run();
  return open_day_patrol_behavior_.Update();
}

void OpenDayPatrolActionNode::OnTerminate(roborts_decision::BehaviorState state) {
  open_day_patrol_behavior_.Cancel();
  switch (state) {
...
OpenDayChaseActionNode::OpenDayChaseActionNode(std::string name, roborts_decision::ChassisExecutor *&chassis_executor,
                                   const roborts_decision::Blackboard::Ptr &blackboard_ptr,
                                   const std::string &proto_file_path) :
  ActionNode::ActionNode(name, blackboard_ptr),
  open_day_chase_behavior_(chassis_executor, blackboard_ptr_, proto_file_path) {}

void OpenDayChaseActionNode::OnInitialize() {
  //ROS_INFO("%s %s", name_.c_str(), __FUNCTION__);
  //chase_behavior_.Run();
}

BehaviorState OpenDayChaseActionNode::Update() {
  open_day_chase_behavior_.Run();
  return open_day_chase_behavior_.Update();
}

void OpenDayChaseActionNode::OnTerminate(roborts_decision::BehaviorState state) {
  open_day_chase_behavior_.Cancel();
