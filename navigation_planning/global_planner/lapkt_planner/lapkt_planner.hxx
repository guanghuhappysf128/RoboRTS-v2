 ...
 ***************************************************************************/
#ifndef ROBORTS_PLANNING_GLOBAL_PLANNER_LAPKT_PLANNER_H
#define ROBORTS_PLANNING_GLOBAL_PLANNER_LAPKT_PLANNER_H
...
#include "proto/lapkt_planner_config.pb.h"
...
namespace roborts_global_planner{
     ...
     */
    class LapktPlanner : public GlobalPlannerBase {
      ...
      public:
         ...
         */
        LapktPlanner(CostmapPtr costmap_ptr);
        virtual ~LapktPlanner();
        /**
      ...
      private:
        ...
        struct Compare {
          bool operator()(const int &index1, const int &index2) {
            return LapktPlanner::f_score_.at(index1) > LapktPlanner::f_score_.at(index2);
          }
        };
        ...
        std::vector<LapktPlanner::SearchState> state_;

        bool _initialized;
        lapkt::MapParameters _map_params;
        lapkt::OccupancyMap _occ_map;
        std::shared_ptr<global_planner::OrientationFilter> _orientation_filter;
        bool _old_navfn_behavior;
        float _convert_offset;

      protected:
        bool extract_plan(const std::vector<lapkt::State>& path,
                  const geometry_msgs::PoseStamped& goal,
                  std::vector<geometry_msgs::PoseStamped>& plan);
        bool world_to_map(double wx, double wy, double& mx, double& my);
        bool world_to_map(lapkt::Point& pt, CostmapPtr costmap_ptr);
        void map_to_world(double mx, double my, double& wx, double& wy);

    };

    std::vector<int> LapktPlanner::f_score_;
...
#endif // ROBORTS_PLANNING_GLOBAL_PLANNER_LAPKT_PLANNER_H
