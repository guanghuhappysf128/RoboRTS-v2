...
void GimbalContrl::Transform(cv::Point3f &position, float &pitch, float &yaw) {
  pitch =
      -GetPitch((position.z + offset_.z) / 1000, -(position.y + offset_.y) / 1000, 15) + (float)(offset_pitch_ * 3.1415926535 / 180);
  //yaw positive direction :anticlockwise
  ROS_INFO("distance is %f; height is %f, target position x = %f, y = %f", (position.z + offset_.z) / 100, -(position.y + offset_.y) / 100, position.z, position.y);
